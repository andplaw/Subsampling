function [D] = rbffd_gpu_weights(Xd,X,rbfset)
%Input:
%Xd             data points (:,1:3)
...
% D = rbffd_gpu_weights(Xd,X,rbfset);
% dsize = size(Xd,1);
% esize = size(X,1);
...
for nchunk=1:chunks
    ...
    %Rearrange nearest neighbor IDs to be suitable for GPU computations
    IDx=reshape(ID',1,n,[]);%3-dimensional array of indices of indices
    Xe = reshape(gpuArray(Xe'),1,[],Ne);
    ...
    nullC=zeros(pterms,pterms,Ne,'double','gpuArray'); %null matrix
    C=[A P; pagetranspose(P) nullC]; %full collocation matrix
    L = [LA;LP]; %setup RHS


    w = pagefun(@mldivide,C,L); %compute weights by solving Cw = L

    NaNid = find(isnan(w(1,1,:))); %mldivide may introduce NaN weights
    while ~isempty(NaNid) %re-compute until no NaNs are present (workaround for now)
        w(:,:,NaNid) = pagefun(@mldivide,C(:,:,NaNid),L(:,:,NaNid));
        NaNid = find(isnan(w(1,1,:)));
    end

    if strcmp(Did,'D')
        if dim == 1
            D.dx = [D.dx;gather(reshape(w(1:n,1,:),n,Ne,1)')];
            D.L = [D.L;gather(reshape(w(1:n,2,:),n,Ne,1)')];
            D.id = [D.id;ID];

        elseif dim == 2
            D.dx = [D.dx;gather(reshape(w(1:n,1,:),n,Ne,1)')];
            D.dy = [D.dy;gather(reshape(w(1:n,2,:),n,Ne,1)')];
            D.L = [D.L;gather(reshape(w(1:n,3,:),n,Ne,1)')];
            D.id = [D.id;ID];

        elseif dim == 3
            D.dx = [D.dx;gather(reshape(w(1:n,1,:),n,Ne,1)')];
            D.dy = [D.dy;gather(reshape(w(1:n,2,:),n,Ne,1)')];
            D.dz = [D.dz;gather(reshape(w(1:n,3,:),n,Ne,1)')];
            D.L = [D.L;gather(reshape(w(1:n,4,:),n,Ne,1)')];
            D.id = [D.id;ID];
        end
    elseif strcmp(Did,'I')
        D.I = [D.I;gather(reshape(w(1:n,1,:),n,Ne,1)')];
        D.id = [D.id;ID];
    end
